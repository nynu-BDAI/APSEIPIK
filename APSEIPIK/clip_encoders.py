...
class CustomTextEncoder1(torch.nn.Module):
    ...
    def __init__(self, clip_model, device, dtype):
        super(CustomTextEncoder1, self).__init__()
        self.dtype = dtype
        self.clip_model = clip_model
...
class CustomTextEncoder(torch.nn.Module):
    ...
    def forward(self, text_feature,text_prefix):
        """The forward function to compute representations for the prompts.
        :param class_embedding: These are the vectors for class names
        :param labels: tensor of labels (already preprocessed without _)
        :param enable_pos_emb: We set this to True since we want to account for the order
        Returns:
            torch.Tensor: the vector representation of the prompt.
        """
        text_feature=text_feature.unsqueeze(dim=1) #升维
        text_prefix=torch.mean(text_prefix,dim=1,keepdim=True)
        combine_feature=torch.cat([text_prefix,text_feature],dim=1)
        input_transformer_features = combine_feature.type(self.dtype)
        #input_transformer_features = text_feature.type(self.dtype)
        #padding
        batch,num_token,dim=input_transformer_features.size()
        pad_len = 77 - num_token
        input_transformer_features = F.pad(input_transformer_features , (0, 0,  pad_len,0))
        x = input_transformer_features + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)
        # log.info(f'DEVICE: {type(self.device)}')
        if torch.cuda.is_available():
            # log.info('WRONG CPU')
...
#text_token_embedding 和 prompt 交互（attention）时使用
class CustomTextEncoder_textwithprompt(torch.nn.Module):
    """This class is adapted from the codebase of "Learning to Compose Soft Prompts for Compositional Zero-Shot Learning"
    https://github.com/BatsResearch/csp/blob/main/clip_modules/text_encoder.py"""

    def __init__(self, clip_model, device, dtype):
        super(CustomTextEncoder_textwithprompt, self).__init__()
        self.dtype = dtype
        self.clip_model = clip_model
        self.transformer = clip_model.transformer
        self.positional_embedding = clip_model.positional_embedding
    ...
    def forward(self, text_feature_withprompt):

        input_transformer_features = text_feature_withprompt.type(self.dtype)
        x = input_transformer_features + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)
        # log.info(f'DEVICE: {type(self.device)}')
        if torch.cuda.is_available():
...
class ImageEncoder(nn.Module):
    ...
    def forward(self, image):
        encoded_image = self.clip_model.encode_image(image)
...
class CustomVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        image_prefix: torch.Tensor, #prompt
        pos_emb=True,
        deep_embs=None,
    ):
        ...
        #image_prefix = image_prefix.expand(x.shape[0], -1, -1).cuda()
