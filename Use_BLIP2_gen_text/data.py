...
class CocoDataset(data.Dataset):
    ...
    def __init__(self, root, json, max_len, useDeepseek, ids=None, split=None, ):
        ...
        """
        self.split = split
        self.cache_dir = os.path.join(os.path.dirname("/mnt/Data/wangshilong/self_datasets/coco/images/train2014"),
                                      "BLIP2_caption_cache")
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            ...
            if captions is None:
                global_text_query = "What does this image describe from a global perspective?"
                background_text_query = "What information is depicted in the background of this image?"
                entity_text_query = "What entities are present in this image?"
    ...
    def getImageDecription(self, image_path, text_query):
        ...
        inputs = self.processor(image, text_query, return_tensors="pt").to(self.devices, torch.float16)
...
class FlickrDataset(data.Dataset):
    ...
    def __init__(self, root, json, split, vocab, max_len, use_deepseek):
        self.cache_dir = os.path.join(os.path.dirname("PATH TO DATASET"),
                                      "BLIP2_caption_cache")
        os.makedirs(self.cache_dir, exist_ok=True)
        ...
        self.dataset = jsonmod.load(open(json, 'r'))[
            'images']  # ["sentids":[],"imageid":0,"sentences":[5个句子信息],"split":"train","filename":"xxxx.jpg"]
        self.ids = []
...
def collate_fn_bert_useDeepseek(data):
    image, original_caption, original_input_ids, global_caption, global_input_ids, background_caption, background_input_ids, entity_caption, entity_input_ids, index = zip(
        *data)
...
class MscocoDataModule(LightningDataModule):
    def __init__(self, _config, dist=False):
        ...
        self.data_path = os.path.join(_config["data_root"],
                                      _config['datasets'])
